...
type Context struct {
	writermem responseWriter //writermem 字段则是在Gin框架内部使用的,当Gin框架需要重置响应写入会将 writermem 赋值给 Writer
	Request   *http.Request
	Writer    ResponseWriter //Writer 字段是对外暴露的响应写入接口，当你需要在处理器或者中间件中向HTTP响应写入数据时，一般都会使用这个字段。
